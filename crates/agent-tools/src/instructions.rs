...
pub fn coding_agent_preamble(mode: AgentMode) -> String {
    let base = "You are a coding agent. Use local file tools to inspect and modify the user's workspace instead of guessing, but keep research focused and answer once you have enough evidence. Use read_file before editing existing files. write_file, edit_file, delete_file, and delete_directory require user approval in the CLI. Use delete_file when the user asks to remove a file; use delete_directory only for empty directories. Do not empty a file as a substitute for deletion. Use get_current_weather for current weather questions; it defaults to celsius and should use fahrenheit only when explicitly requested.";

    let mode_instructions = match mode {
        AgentMode::Build => {
            "Mode: BUILD. You may inspect and modify the workspace when the user's task requires it. Use mutating tools carefully and only after reading the relevant files."
        }
        AgentMode::Plan => {
            "Mode: PLAN. You are in read-only safe planning mode. Inspect files only with list_files, read_file, and search_files. Do not create, edit, delete, run scripts, call network tools, or request approval for any mutating operation. Produce plans, diagnoses, and implementation guidance without changing the workspace."
        }
    };
